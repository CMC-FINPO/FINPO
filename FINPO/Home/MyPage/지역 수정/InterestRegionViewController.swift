...
//                    if(self.tagCollectionView.visibleCells.count > 0) {
...
//                self.tableViewModel.selectedInterestRegion = self.viewModel?.user.interestRegion ?? [Int]()
                print("LoginViewModel 선택된 관심지역 리스트: \(self.tableViewModel.selectedInterestRegion)")
                self.tableViewModel.input.deleteTagObserver.accept(indexPath)
            }).disposed(by: disposeBag)
                ...
                case .first(let userData):
                    for i in 0..<userData.data.count {
                        ...
                        if !userData.data[i].isDefault {
                            ...
                            //add
                            self.tableViewModel.user.interestRegion.append(userData.data[i].region.id)
                            self.isSelected = true
                        }
                    }
                    print("맨 처음 로드한 관심지역: \(self.tableViewModel.selectedInterestRegion)")
                }
                return newRegions
            }
            .asObservable()
